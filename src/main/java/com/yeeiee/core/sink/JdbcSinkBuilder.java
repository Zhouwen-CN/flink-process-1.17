...
public abstract class JdbcSinkBuilder<OUT> implements SinkBuilder<OUT>, CatalogTable {
    ...
    public void build(Context context, DataStream<OUT> input) {
        ...
        val fullTableName = CommonUtil.getFullTableName(this.catalog(), this.database(), tableName);
        jdbcOptions.setTableName(fullTableName);
        ...
        if (this.isCombineSink()) {
            log.info("Use JdbcSink combine");
            statementSet.addInsert(fullTableName, table);
        } else {
            log.info("Use JdbcSink separate");
    ...
    private String getCreateJdbcTableSql(DataStream<?> input, JdbcOptions jdbcOptions) throws BasicException {
        ...
        sql.append("CREATE TABLE ").append(jdbcOptions.getTableName()).append(" (").append(columnNamesWithType);

        // 如果主键不为空, 则添加
        if (!primaries.isEmpty()) {
            sql.append(", ").append("PRIMARY KEY (").append(primaryKeys).append(") NOT ENFORCED ");
        }

        sql.append(") WITH ( 'connector' = 'jdbc', 'url' = '").append(jdbcOptions.getUrl()).append("', 'driver' = '").append(jdbcOptions.getDriverName()).append("', 'table-name' = '").append(jdbcOptions.getTableName()).append("', 'username' = '").append(jdbcOptions.getUsername()).append("', 'password' = '").append(jdbcOptions.getPassword()).append("' )");
